...
const startMatchAsync = async (matchId) =>{
    ...
    const match = await httpClient.post('https://localhost:5001/api/Match/StartNewRound',data);
    console.log(match);
    return match;
}
...
const sendRetireAsync = async () =>{
    console.log("Send Retire");
     ...
     if(sessionStorage.getItem("RoundData"))
     {
        ...
        console.log(data);
        const response = await httpClient.post('https://localhost:5001/api/Match/SendRetire',data);
